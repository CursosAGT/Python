...
def Ej_ya_hechos(x):
	...
	texto 	
		w	write	Si exite el archivo lo borra y pone el nuevo contenido. Si no existia, lo crea
		a	append	Agrega (al final) los datos deseado
		...
		read()
	binario
		wb	write binario	Si exite el archivo lo borra y pone el nuevo contenido. Si no existia, lo crea para datos binario
		rb	read binario	Lectura de un archivo  binario
	...
	╚═════════════════════════════════════════════════════════════════════════════╝
	""");
	print("Abre el navedador en el directorio de trabajo de python.\n Ve los archivos y las fechas\ncontinuar???");
	pausa()
	...
	print("salida de datos a otro archivo  write w ");
	...
	#################################################################
	#IO_ext Ej_02 ;
	print(" agrego datos a un archivo si existe sino lo creo\n Salida de datos a otro archivo append");
	...
	archivo_de_texto.close()
	pausa();
	print(texto_a_memoria.upper());
	pausa();
	print(texto_a_memoria.lower());
	pausa();
	dato = input("Ingrese el dato a buscar en el archivo text.txt :")
	if dato in texto_a_memoria:
		print (f"Si esta '{dato}' en el archivo")
	else:
		print (f"no se encuentra '{dato}' en la linea");
	#buscar en los ejemplos de arrays como encontrarla posicion, cuantas veces se repite
	nuevo(3);
	print("""
	╔═════════════════════════════════════════════════════════════════════════════╗
	║                                                                             ║
	║                               metodo 'WITH'                                 ║
	║                              ---------------                                ║
	║              Vean que el archivo solo se mantiene abierta nuentras          ║
	║                 se lee o escribe, luego se cierra inmediatamente            ║
	╚═════════════════════════════════════════════════════════════════════════════╝
	""");

	print("ingreso lectura de bloque de datos desde otro archivo read");
	with open("text.txt","r") as archivo_de_texto:#						abre el archivo text.txt para lectura en bloque
		texto_a_memoria=archivo_de_texto.read()
		print(texto_a_memoria);
	pausa();
	print(texto_a_memoria.upper());
	...
	for lineas in archivo_de_texto.readlines():
		print(lineas);
		dato = "wiki";
		if dato in lineas:
			print (f"Si esta '{dato}' en la linea")
		else:
			print (f"no se encuentra '{dato}' en la linea");
		print("----");
	print("-------------------------------------------------------------------");
	...
	╚═════════════════════════════════════════════════════════════════════════════╝
	""");
	import pickle		#<<<<<<<<<<<<<<<<<<<<<<<ver de importar librerias
	from io import *	#<<<<<<<<<<<<<<<<<<<<<<<ver de importar librerias
	...
	for lineas in archivo_de_binario.readlines():
		print(lineas);
		print("----");
	archivo_de_binario.close()
	del archivo_de_binario
	...
	╚═════════════════════════════════════════════════════════════════════════════╝
	""");
	import json			#<<<<<<<<<<<<<<<<<<<<<<<ver de importar librerias
	from io import *	#<<<<<<<<<<<<<<<<<<<<<<<ver de importar librerias
...
#IO_ext Ej_18;
import os
print("Compruebo la existencia del archivo y el path donde encontrarlo");
archivo_de_texto_nombre = "Xtext.txt";
#os.path.remove(archivo_de_texto_nombre);
#os.path.mkdir(directorio);

if os.path.isfile(archivo_de_texto_nombre) == False:
	print(f"El archivo {archivo_de_texto_nombre} 'NO' existe en el directorio solicitado");
else:
	print(f"El archivo {archivo_de_texto_nombre} si existe\n procedemos a trabajar con el");
	archivo_de_texto=open(archivo_de_texto_nombre,"r");#						abre el archivo text.txt para lectura en bloque
	texto_a_memoria=archivo_de_texto.read()
	print(texto_a_memoria);
	archivo_de_texto.close()
print("Continuar?");
